...
func (c *rootCmd) Run(ctx context.Context, kongCtx *kong.Context) error {
	...
	if c.OutputFormat == "action" {
		logHandler = actionlogger.NewActionHandler(os.Stdout, &slogOpts)
	}
	ctx = actionlogger.WithLogger(ctx, slog.New(logHandler))
	if c.GenerateAction {
...
func (c *rootCmd) runRelease(ctx context.Context) (errOut error) {
	logger := actionlogger.GetLogger(ctx)
	logger.Info("starting runRelease")
	logger.Info("multi line log message\nline 2\nline 3")
	logger.Info(`
{
